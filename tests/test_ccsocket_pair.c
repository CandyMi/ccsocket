...
int main(void)
{
    ...
    ccsocket_stcode_t rc = ccsocket_send(sv[0], msg_a, len_a, &n);
    assert(rc == CC_OPCODE_OK);
    assert(n == (int)len_a);

    n = 0;
    rc = ccsocket_recv(sv[1], buf, sizeof(buf), &n);
    assert(rc == CC_OPCODE_OK);
    assert(n == (int)len_a);
    buf[n] = '\0';
    ...
    rc = ccsocket_send(sv[1], msg_b, len_b, &n);
    assert(rc == CC_OPCODE_OK);
    assert(n == (int)len_b);

    n = 0;
    rc = ccsocket_recv(sv[0], buf, sizeof(buf), &n);
    assert(rc == CC_OPCODE_OK);
    assert(n == (int)len_b);
    buf[n] = '\0';
